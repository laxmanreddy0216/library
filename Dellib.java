...
import java.sql.*;
public class Dellib extends HttpServlet
{
	Connection con=null;
		...
		public void myMethod(HttpServletRequest req,HttpServletResponse res)throws ServletException,IOException
	{
			try
			{
					...
					ps=con.prepareStatement("delete from librarian where lid = ?");
					ps.setString(1,a2);
						...
						if(k!=0)
				{
							...
							RequestDispatcher rd=req.getRequestDispatcher("dellib3.html");
								rd.forward(req,res);
				}
						else
							pw.println("failed");
